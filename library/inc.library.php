<?php
//TODO: Pengaturan tanggal komputer
date_default_timezone_set("Asia/Jakarta");


//TODO: Fungsi untuk membuat kode automatis
function buatKode($tabel, $inisial)
{
	global $koneksidb;

	$sql		= "SELECT * FROM $tabel";
	$struktur	= mysqli_query($koneksidb, $sql);
	$field		= mysqli_fetch_field_direct($struktur, 0);
	$panjang	= mysqli_fetch_field_direct($struktur, 0);

	$qry	= mysqli_query($koneksidb, "SELECT MAX(" . $field->name . ") FROM " . $tabel);
	//! Mengecek data apakah berjalan atautidak nya
	//! die(var_dump($field));
	...
	$angka++;
	$angka	= strval($angka);
	//! Mengecek data apakah berjalan atau tidak nya
	//! die(var_dump($angka));
	$tmp	= "";
	for ($i = 1; $i <= ($panjang->max_length - strlen($inisial) - strlen($angka)); $i++) {
		$tmp = $tmp . "0";
	}
	return $inisial . $tmp . $angka;
}

//TODO: Fungsi untuk membalik tanggal dari format Indo (d-m-Y) -> English (Y-m-d)
...
//TODO: Fungsi untuk membalik tanggal dari format English (Y-m-d) -> Indo (d-m-Y)
...
//TODO: Fungsi untuk membuat format rupiah pada angka (uang)
...
//TODO: Fungsi untuk format tanggal, dipakai plugins Callendar
